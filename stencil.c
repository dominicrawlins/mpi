  ...
  if(size == 1){
    ...
    if(rank == MASTER){
      ...
      if(rank == size - 1){
        ...
        for(short t = 0; t < niters; t++){
          MPI_Sendrcv(&image[last*nx], nx, MPI_FLOAT, rank+1, 1,
            &image[(first-1)*nx], nx, MPI_FLOAT, rank-1, 1, MPI_COMM_WORLD, &status);

            MPI_Sendrcv(&image[first*nx], nx, MPI_FLOAT,rank-1, 1,
              &image[(last+1)*nx], nx, MPI_FLOAT, rank+1, 1, MPI_COMM_WORLD, &status);
